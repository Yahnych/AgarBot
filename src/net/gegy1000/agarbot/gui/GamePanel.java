...
public class GamePanel extends JPanel
{
    ...
    public void draw(Graphics2D g, boolean neural)
    {
        ...
        g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, neural ? RenderingHints.VALUE_ANTIALIAS_OFF : RenderingHints.VALUE_ANTIALIAS_ON);
...
//            System.out.println(Arrays.toString(playerX) + ", " + world.cameraX);
            ...
            for (Cell cell : cells)
            {
                drawCell(smallestPlayer, cell, g, world.cameraX, world.cameraY, player.contains(cell), neural);
            }
        }
        ...
        if (!neural)
        {
            ...
            if (controller != null)
            {
                ...
                g.drawString("Current Fitness: " + world.getFitness(), 100, 325);
                ...
                if (inputs != null)
                {
                    for (int y = 0; y < height; y++)
                    {
                        for (int x = 0; x < width; x++)
                        {
                            ...
                            int colour = (int) ((inputs[baseIndex] + 1) * 128);
    ...
    private void drawNeuralInput(Graphics2D g, int x, int y, Color color) //TODO use actual screen input
    {
        x *= 5;
        y *= 5;
    ...
    public void drawCell(Cell smallestPlayer, Cell cell, Graphics2D g, int cameraX, int cameraY, boolean isPlayer, boolean neural)
    {
        int actualX = cell.x;
        int actualY = cell.y;
